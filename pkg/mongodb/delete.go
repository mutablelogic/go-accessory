...
func (collection *collection) DeleteMany(ctx context.Context, filter ...Filter) (int64, error) {
	// Check for collection
	if collection.Collection == nil {
		return -1, ErrOutOfOrder
	}
	...
	// Trace
	ctx, matched, modified := trace.WithCollection(ctx, trace.OpDeleteMany, collection.Database().Name(), collection.Name())
	defer trace.Do(ctx, collection.traceFn, time.Now())

	// Perform the delete
	result, err := collection.Collection.DeleteMany(ctx, and(filter...), &options.DeleteOptions{})
	if err != nil {
		return -1, err
	} else {
		*matched, *modified = result.DeletedCount, result.DeletedCount
